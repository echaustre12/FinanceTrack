...
function IncomeFormModal({ paymentMethods, onClose, onSave }) {
  ...
  const ready = form.amount && form.description && form.date && form.paymentMethodId;
  ...
  return (
    <Modal title="Registrar ingreso" onClose={onClose}>
      ...
      <input className="clay-input" type="number" value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} placeholder="Ej. 3200000" />
      <label className="field-label">Descripción</label>
      <input className="clay-input" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Ej. Pago de salario" />
